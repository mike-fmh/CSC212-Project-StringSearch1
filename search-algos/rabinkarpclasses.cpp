...
rabin_karp::rabin_karp(string text, string query, int numLetters, int prime) {
	...
	// then modulo h by our prime number to make the prime our range of the hash function
...
void rabin_karp::search() {
	...
	for (int i = 0; i < 1 + this->text.size() - this->query.size(); i++) {
		if (this->windowhash == this->queryhash) {
			...
			// but hashing doesn't always give unique values, so sometimes different strings will hash to the same value
			// to make sure it's a match, check each letter of the query string against the corresponding letter of the window
			int j;
